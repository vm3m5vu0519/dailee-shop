...
$('body > .header').load('header.html', function () {
    $.ajax({
        ...
        success: function (result) {
            if (result.uname) {
                $('.login_info').html('<li>&nbsp;歡迎:' + result.uname + ' <a href="logout.html" title="退出登入">退出</a><b>|</b></li><li><a href="uc_basic.html" title="用戶中心">用戶中心</a></li>');
                $('[href="logout.html"]').click(function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.code === 200) {
                                alertMsg('<h4>退出成功</h4>點擊確定重新返回登入頁面');
                                $('#alertMsg').on('click', '#alertMsg_btn1 cite', function (e) {
                                    e.preventDefault();
                                    location.href = 'login.html';
                                })
                            } else {
                                alertMsg('登入退出失敗！原因：' + result.msg);
                            }
                        }
                    })
                });
