...
jQuery(".qfyuser-pic-upload").each(function(){
	...
	var keepname = 0;
	if(jQuery(this).closest(".video_warp").length>0){
		keepname = 1;
	}
	jQuery(this).uploadFile({
		...
		formData: {tmp_file_id: "file"+qfyuser_GetCookiePara()+unique_id+"-"+fkey,"keepname":keepname},
		...
		onSuccess:function(files,data,xhr){
			...
			jQuery.ajax({
				...
				success:function(data){
					...
					if(statusbar.closest("#buddypress").length==1 && filetype == 'file') {

						statusbar.prev().fadeIn( function() {
							data.response = data.response.replace("</a>","</a>&nbsp;&nbsp;<span style='color:red;cursor: pointer;' class='qfyuser-remove-file-item'>删除</span>");
							statusbar.parents('.qfyuser-input').prepend( data.response );
							statusbar.hide();
						});
						var v = statusbar.closest('.qfyuser-input').find('input:hidden').val();
						if(v==""){
							v = src;
						}else{
							v = src+","+v;
						}
						statusbar.closest('.qfyuser-input').find('input:hidden').val( v );
						statusbar.closest('.qfyuser-input').find('.qfyuser-pic-none').hide();
...
function qfyuser_password_strength_meter(element){
		...
		if (meter.length > 0 ) {
			if  ( password.length < 8 ) {
				...
				//meter_text.html( '密碼太短' );
				return 0;
			} else if ( SAME.test(password) ) {
				meter.find('.strength-plain').removeClass('fill');
				meter.find('.strength-plain:eq(0)').addClass('fill');
				meter_text.html( meter_data['very_weak'] );
				//meter_text.html( '密碼強度非常弱' );
				return 1;
			} else if ( lower && upper && digit && special ) {
				meter.find('.strength-plain').removeClass('fill');
				meter.find('.strength-plain').addClass('fill');
				meter_text.html( meter_data['very_strong'] );
				//meter_text.html( '密碼強度非常強' );
				return 5;
			} else if ( lower && upper && digit || lower && digits || upper && digits || special ) {
				meter.find('.strength-plain').removeClass('fill');
				meter.find('.strength-plain:eq(0),.strength-plain:eq(1),.strength-plain:eq(2),.strength-plain:eq(3)').addClass('fill');
				meter_text.html( meter_data['strong'] );
				//meter_text.html( '密碼強度強' );
				return 4;
			} else if (lower && upper || lower && digit || upper && digit) {
				meter.find('.strength-plain').removeClass('fill');
				meter.find('.strength-plain:eq(0),.strength-plain:eq(1),.strength-plain:eq(2)').addClass('fill');
				meter_text.html( meter_data['good'] );
				//meter_text.html( '密碼強度一般' );
				return 3;
			} else {
				meter.find('.strength-plain').removeClass('fill');
				meter.find('.strength-plain:eq(0),.strength-plain:eq(1)').addClass('fill');
				meter_text.html( meter_data['weak'] );
				//meter_text.html( '密碼強度弱' );
				return 2;
			}
		}
}
...
jQuery(document).ready(function() {
	...
	**/
	jQuery(document).on('submit', '.qfyuser form:not(.qfyuser-search-form,.qfyuser-not-submit)', function(e){
		if(self!=top && typeof parent.jAlert =="function" && !jQuery("body").hasClass("allowEdit")){
			jAlert("在編輯模式下，不允許登錄，註冊，修改內容，請到預覽模式下使用。");
			return false;
		}
		...
		form.closest(".qfy-element").removeClass("qsa").css("animation-duration","0.1s");
		}
		...
		if ( form.data('action') != 'login' &&  form.data('action') != 'verifyrealname' && form.data('action') != 'phone'  && form.data('action') != 'askinfo' && form.data('action') != 'reset' &&  form.data('action') != 'delete' ) {
			...
			if (form.find('.qfyuser-warning').length > 0){
				...
				if(login_animale!="1"){
					form.closest(".qfy-element").addClass('qfe_qfy_swing-3 qsa');
				}
				return false;
			} else {
				form.find('.qfyuser-section').each(function(){
		...
		function user_ajax_form_submit(){
			jQuery.ajax({
				url: qfyuser_ajax_url,
		...
		if(form.find(".g-recaptcha-response").length>0 && grecaptcha){
			grecaptcha.ready(function() {
				let key = form.find(".g-recaptcha-response").attr("data-key");
				grecaptcha.execute(key, {action: 'submit'}).then(function(token) {
					form.find(".g-recaptcha-response").val(token);
					user_ajax_form_submit()
				})
			});
		}else if(form.find('[name="captcha-val-key"]').length>0){
			if(window.slidercatchar_is_robot=="no"){
				user_ajax_form_submit();
			}else{
				let captchaObj = [];
				qfyuser_end_load( form );
				form.find('[name="captcha-val-key"]').each(function(i){
	...
	**/
	...
	jQuery(document).on('click', '.qfyuser .qfyuser-remove-file-item', function(e){
		var p = jQuery(this).closest('.qfyuser-input');
		if ( jQuery(this).closest('.qfyuser-file-input').length) {
			jQuery(this).closest('.qfyuser-file-input').remove();
		}
		var arr = new Array();
		p.find('.qfyuser-file-input>a').each(function(){
			arr.push($(this).attr("href"));
		});

		var v = arr.join(",");
		p.find('input:hidden').val(v);
		jQuery(this).fadeOut();
	...
	jQuery.ajax({
		...
		error: function(request) {
			...
			if (element.parent().find('.qfyuser-warning').length==0 ) {
				element.addClass('warning').removeClass('ok');
				element.after('<div class="qfyuser-warning"><i class="qfyuser-icon-caret-up"></i>提交失敗</div>');
				element.parent().find('.qfyuser-warning').css({'top' : '0px', 'opacity' : '1'});
			} else {
				element.parent().find('.qfyuser-warning').html('<i class="qfyuser-icon-caret-up"></i>提交失敗' );
				element.parent().find('.qfyuser-warning').css({'top' : '0px', 'opacity' : '1'});
			}
		},
		success: function(data) {
...
function tominuto1(obj){
	...
	if(text1 && text2){
		...
		if(jQuery(obj).val()>0 && jQuery(obj).val()!="發送" && jQuery(obj).val()!="獲取" && jQuery(obj).val()!="發送成功"){
			jQuery(obj).val(jQuery(obj).val()-1);
		}else{
			jQuery(obj).removeClass("begining");
			jQuery(obj).val(text1);
